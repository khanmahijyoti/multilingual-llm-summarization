...
def call_openrouter_api(prompt, api_key, model="meta-llama/llama-3.3-70b-instruct", timeout=30):
    ...
    headers = {
        ...
        "X-Title": "English Dataset Summarization Evaluation"
    }
    payload = {
        "model": model,
        "messages": [{"role": "user", "content": prompt}],
...
def main():
    parser = argparse.ArgumentParser(description="Unified Evaluator for English XL-Sum using Groq + OpenRouter Keys")
    parser.add_argument("--data_path", type=str, default=r"E:\sounds\english_XLSum_v2.0\english_test.jsonl", help="Path to English XLSum test jsonl file")
    parser.add_argument("--keys_file", type=str, default=r"E:\sounds\bengali_XLSum_v2.0\api keys", help="File containing Groq and OpenRouter API keys")
    parser.add_argument("--limit", type=int, default=1012, help="Maximum number of items to evaluate")
    parser.add_argument("--output_csv", type=str, default=r"E:\sounds\english_XLSum_v2.0\groq_llama70b_1012_english_results.csv", help="CSV file to load/save predictions")
    ...
    print(f"[+] Loaded {len(dataset)} English dataset items for {display_model} evaluation.")
    ...
    # Standard English ROUGE Scorer with Porter stemmer
    ...
    for item in tqdm(dataset, desc=f"Evaluating English {display_model}"):
        item_id = str(item['id'])
    ...
    print("\n==================================================")
    print(f"  English {display_model} ROUGE Benchmark Results   ")
    print("==================================================")
