...
def call_openai_api(prompt, system_prompt, model, api_key, timeout=30):
    global total_input_tokens_estimate, total_output_tokens_estimate
    ...
    # Handle API differences for gpt-5/o1/luna models
    ...
    # Estimate input tokens (rough estimation: 1 word ~ 1.3 tokens, plus 1 Bengali char ~ 1.5 tokens)
    input_text = system_prompt + " " + prompt
    total_input_tokens_estimate += int(len(input_text) * 1.5)
    ...
    with urllib.request.urlopen(req, timeout=timeout) as response:
        ...
        total_output_tokens_estimate += int(len(content) * 1.5)
        return content
...
def main():
    global openai_keys
    parser = argparse.ArgumentParser(description="Evaluate a batch of 50 summaries using BanglaSummEval")
    ...
    parser.add_argument("--model", type=str, default="gpt-5.6-luna", help="Model to run QG/QA/NER")
    parser.add_argument("--threshold", type=float, default=85.0, help="BERTScore-Recall match threshold")
    parser.add_argument("--output_csv", type=str, default="banglasummeval_deepseek_50_results.csv", help="Output results CSV path")
            ...
            with open(args.keys_file, "r", encoding="utf-8") as kf:
                for line in kf:
                    ...
                    if not key or key.startswith("#"):
                        continue
                    if key.startswith("sk-proj-") or (key.startswith("sk-") and not key.startswith("sk-or-v1-")):
                        openai_keys.append(key)
                        
    if not openai_keys:
        raise ValueError("No valid OpenAI API keys loaded.")
        
    print(f"[+] Loaded {len(openai_keys)} OpenAI keys.")
    print(f"[+] Evaluating batch of {args.limit} samples using {args.model}")
    print(f"[+] Predictions: {args.results_csv}")
    print(f"[+] Local BERTScore running on: {device}")
    ...
    df_preds = pd.read_csv(args.results_csv)
    
    # Setup/Resume Output CSV
    results = []
    completed_ids = set()
    if os.path.exists(args.output_csv):
        try:
            ...
            print(f"[+] Resuming: Found {len(completed_ids)} already-evaluated samples.")
        except Exception as e:
            print(f"[!] Warning reading old CSV: {e}")
    ...
    for i in range(args.limit):
        ...
        # Find prediction matching sample ID
        matched_rows = df_preds[df_preds['id'].astype(str) == sample_id]
        if matched_rows.empty:
            print(f"[!] Sample ID {sample_id} not found in predictions CSV. Skipping.")
            continue
            
        generated_summary = matched_rows.iloc[0]['generated']
        source_document = sample['text']
        
        print(f"\n[{i+1}/{args.limit}] Evaluating ID: {sample_id}...")
        
        try:
            prec, rec, f1 = run_pipeline(source_document, generated_summary, args.model, "openai", args.threshold)
            ...
            print(f"    [!] Error evaluating sample index {i} (ID: {sample_id}): {e}")
            # wait and retry once
            time.sleep(5)
    ...
    if results:
        ...
        avg_prec = df_final['precision'].mean()
        avg_rec = df_final['recall'].mean()
        avg_f1 = df_final['banglasummeval_f1'].mean()
        
        elapsed = time.time() - t0
        print("\n" + "="*60)
        print("BanglaSummEval Batch Evaluation Summary (Deepseek V4 Flash)")
        print("="*60)
        print(f"  - Total Evaluated Samples: {len(results)}")
        print(f"  - Average Precision (Factuality): {avg_prec:.2f}%")
        print(f"  - Average Recall (Coverage): {avg_rec:.2f}%")
        print(f"  - Average BanglaSummEval F1: {avg_f1:.2f}%")
        print(f"  - Total Elapsed Time: {elapsed:.2f} seconds")
        print(f"  - Estimated Input Tokens Used: {total_input_tokens_estimate:,}")
        print(f"  - Estimated Output Tokens Used: {total_output_tokens_estimate:,}")
        print("="*60)
    else:
