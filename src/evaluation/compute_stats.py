...
def load_csv_column(filepath, col_name, id_col='id'):
    ...
    with open(filepath, 'r', encoding='utf-8-sig', errors='ignore') as f:
        ...
        for row in reader:
            ...
            if not item_id:
                # try alternative if BOM or variation
                for k in row.keys():
...
def calc_stats(values, B=2000):
    n = len(values)
    if n == 0:
        return {'mean': 0, 'std': 0, 'ci_low': 0, 'ci_high': 0, 'boot_low': 0, 'boot_high': 0, 'n': 0}
    mean = sum(values) / n
    var = sum((x - mean) ** 2 for x in values) / (n - 1) if n > 1 else 0
    std = math.sqrt(var)
    se = std / math.sqrt(n)
    ci_low_analytical = mean - 1.96 * se
    ci_high_analytical = mean + 1.96 * se
    ...
    return {
        ...
        'std': std,
        'se': se,
        'ci_low': ci_low_analytical,
        'ci_high': ci_high_analytical,
        'boot_low': boot_low,
        'boot_high': boot_high,
        'n': n
    }
...
print("="*75)
print("1. BENGALI FACTUALITY (BanglaSummEval BSE-F1)")
print("="*75)
...
print("\n--- Reviewer 1 Direct Question: Gemini 2.5 Flash (58.43) vs LLaMA 3.3 70B (58.28) ---")
...
if res:
    ...
    if res['boot_p'] > 0.05:
        print(">> Finding: The 0.15 difference is NOT statistically significant (p = 0.92 > 0.05). Both models perform equivalently.")
...
print("\n--- English Factuality: DeepSeek V4 Flash (63.45) vs LLaMA 3.3 70B (60.18) ---")
res_en = paired_test(en_fact_data['DeepSeek V4 Flash'], en_fact_data['LLaMA 3.3 70B'])
if res_en:
    print(f"Paired N={res_en['n']}, Mean Diff={res_en['mean_diff']:+.2f}, t={res_en['t_stat']:.3f}, p={res_en['p_val']:.4f}, boot_p={res_en['boot_p']:.4f}")
...
print("\n--- Bengali ROUGE-1: LLaMA 3.3 70B (16.23) vs GPT-4o Mini (15.63) ---")
res_bn_r1 = paired_test(bn_r1_data['LLaMA 3.3 70B'], bn_r1_data['GPT-4o Mini'])
if res_bn_r1:
    print(f"Paired N={res_bn_r1['n']}, Mean Diff={res_bn_r1['mean_diff']:+.2f}, t={res_bn_r1['t_stat']:.3f}, p={res_bn_r1['p_val']:.4f}, boot_p={res_bn_r1['boot_p']:.4f}")

print("\n--- Bengali ROUGE-1: LLaMA 3.3 70B (16.23) vs Gemini 2.5 Flash (15.05) ---")
res_bn_r1_gem = paired_test(bn_r1_data['LLaMA 3.3 70B'], bn_r1_data['Gemini 2.5 Flash'])
if res_bn_r1_gem:
    print(f"Paired N={res_bn_r1_gem['n']}, Mean Diff={res_bn_r1_gem['mean_diff']:+.2f}, t={res_bn_r1_gem['t_stat']:.3f}, p={res_bn_r1_gem['p_val']:.4e}, boot_p={res_bn_r1_gem['boot_p']:.4f}")
...
print("\n--- English ROUGE-1: GPT-5.6 Luna (24.12) vs Gemini 3.1 Flash Lite (22.94) ---")
res_en_r1 = paired_test(en_r1_data['GPT-5.6 Luna'], en_r1_data['Gemini 3.1 Flash Lite'])
if res_en_r1:
    print(f"Paired N={res_en_r1['n']}, Mean Diff={res_en_r1['mean_diff']:+.2f}, t={res_en_r1['t_stat']:.3f}, p={res_en_r1['p_val']:.4e}, boot_p={res_en_r1['boot_p']:.4f}")
